...
export class CalendarComponent implements OnInit {
  ...
  getReservedHoursOfSingleDate(wantedDate: Date) {
    const formattedDate: string = wantedDate
      .toLocaleString('hu-HU')
      .replaceAll('. ', '-')
      .substring(0, 10);
    this.drivingLessonService
      .getReservedHourByDate(
        ...
        next: (response) => {
          ...
          for (let i: number = 0; i < 7; i++) {
            this.days.push({
              ...
              reservedHours: hourCards.filter(
                (hc) =>
                  hc.date.toString().split('T')[0] ===
                  this.datesOfWeek()[i].toISOString().split('T')[0],
              ),
            });
          }
        },
  ...
  getWeekDates(weekNumber: number, year = new Date().getFullYear()): Date[] {
    const jan4 = new Date(year, 0, 4, 12); // dél, hogy ne csússzon időzóna miatt

    const jan4Day = jan4.getDay() || 7; // hétfő=1 ... vasárnap=7
  ...
  changeDateInPhone(isTommorrow: boolean) {
    const weeksDate = this.datesOfWeek().map(
    ...
    if (!weeksDate.includes(nextDate.toDateString())) {
      this.datesOfWeek.set(
        this.getWeekDates(this.getWeekOfYear(nextDate), 2026),
      );
      console.log('');
