...
export class RequestListComponent implements OnInit {
  ...
  makeRows(): (SchoolJoinRequest | DrivingLessonRequest | InstructorJoinRequest)[][] {
    const rows: (SchoolJoinRequest | DrivingLessonRequest | InstructorJoinRequest)[][] = []

    for (let i: number = 0; i < this.requestList.length; i += 2) {
      const row: (SchoolJoinRequest | DrivingLessonRequest | InstructorJoinRequest)[] = []
      for (let j = i; j < i + 2; j++) {
